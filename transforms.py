...
def R2rpy(R):
    ...
    # followed eqn (2.22) in the textbook
...
def R2axis(R):
    ...
    axis_angle = np.array([ang,
                            1/(2*np.sin(ang)) * (R[2,1] - R[1,2]), # TODO - fill out here, each row will be a function of "ang"
...
def R2quat(R):
    ...
    return (1/2) * np.array([np.sqrt(R[0,0] + R[1,1] + R[2,2] + 1),
